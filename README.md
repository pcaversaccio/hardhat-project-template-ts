...
> For deployment on the [zkSync Era](https://era.zksync.io) test network, you must uncomment all zkSync-specific configurations in the [`hardhat.config.ts`](./hardhat.config.ts) file. Afterwards, add your to-be-deployed contract to [`deploy-zksync.ts`](./deploy/deploy-zksync.ts) and run `yarn compile`. Next, fund your deployer account on zkSync Era Testnet and simply run `yarn deploy:zksynctestnet`. Eventually, to verify the contract you can invoke: `npx hardhat verify --network zkSyncTestnet --constructor-args arguments.js <YOUR_CONTRACT_ADDRESS>`.
