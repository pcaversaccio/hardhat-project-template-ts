...
> [!IMPORTANT]
> Truffle has been [sunsetted](https://consensys.io/blog/consensys-announces-the-sunset-of-truffle-and-ganache-and-new-hardhat) by Consensys, but I still keep it in the template as I find it a very valuable tool. Due to the lengthy loading time of [Truffle Dashboard](https://trufflesuite.com/docs/truffle/how-to/use-the-truffle-dashboard/), the module is disabled by default in the [hardhat.config.ts](./hardhat.config.ts) file. If you want to use it, you must uncomment the module import and the `truffle` configuration.
